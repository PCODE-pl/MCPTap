...
EOF
    ...
    for file in proxy.env openrouter.env requesty.env meta.env nano-gpt.env llmtr.env kenari.env unorouter.env runinfra.env nvidia.env opencode.env orcarouter.env aihubmix.env tokenrouter.env inferx.env kilo.env pendra.env vercel.env zenmux.env abliteration-ai.env above.env agentrouter.env agnes.env ai-router.env aiand.env ainetcafe.env aixy.env aki-io.env alibaba.env 302ai.env abacus.env alibaba-cn.env alibaba-coding-plan.env alibaba-coding-plan-cn.env alibaba-token-plan.env alibaba-token-plan-cn.env amazon-bedrock.env ambient.env amd.env anthropic.env anyapi.env arcee.env atomic-chat.env auriko.env azure.env azure-cognitive-services.env bailing.env baseten.env berget.env blueclaw.env bothub.env cerebras.env chutes.env clarifai.env claudinio.env cline-pass.env cloudferro-sherlock.env cloudflare-ai-gateway.env cloudflare-workers-ai.env cohere.env coralbricks.env cortecs.env crof.env crossmodel.env crusoe.env daoxe.env databricks.env deepinfra.env deepseek.env digitalocean.env dinference.env drun.env ebcloud.env echo.env edenai.env empiriolabs.env evroc.env fastrouter.env fireworks-ai.env freemodel.env friendli.env frogbot.env github-copilot.env gmicloud.env google.env google-vertex.env google-vertex-anthropic.env greenpt.env groq.env helicone.env hetzner.env hpc-ai.env huggingface.env hyper.env iflowcn.env impossibl.env inception.env inceptron.env inco.env mcp-intercept.yaml per-model.yaml use_tool_hook.py; do
